...
function registerRequest( player, email, password, password2 )
    ...
    if account == false then
        ...
        local query = Query( "SELECT COUNT( * ) as count FROM users WHERE serial=?", serial )
        ...
        if query and query[ 1 ] and tonumber( query[ 1 ].count ) >= 3 then
            ...
            local admin_level = 1
            Exec( "INSERT INTO users( username, password, email, serial, admin_level ) VALUES ( ?, ?, ?, ?, ? )", username, password, email, serial, admin_level )
...
function loginRequest( player, email, password )
   local username = getPlayerName( player )
   local check = QuerySingle( "SELECT * FROM users WHERE username = ? ", username )
   if check then
       local checkPass = check.password
       local checkEmail = check.email
       if ( checkPass == password ) and ( checkEmail == email ) then
           ...
           outputChatBox( "Неправильный Email или пароль", player, 255,0,0 )
       end
    else
